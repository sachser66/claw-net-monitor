...
function renderSession(node, kind){
  ...
  const used = s.percent_used >= 0 ? (String(Math.max(0, 100 - s.percent_used)) + '%') : '-';
  ...
  return `<div class="${lineClass}">
    ...
    <div class="agentMeta2">${renderKv('tokens:', total)} <span class="sep">|</span> ${renderKv('remaining:', remaining)} <span class="sep">|</span> ${renderKv('left:', used)} <span class="sep">|</span> ${renderKv('fresh:', fresh)}</div>
    ...
    <div class="stack">
      <div class="panel">
        ...
        <div class="agent">
          ...
          <div class="agentMeta2">${renderKv('max left:', `${Math.max(0, 100 - (open.status?.max_percent_used ?? 0))}%`)} <span class="sep">|</span> ${renderKv('cost 7d:', open.usage_cost?.available ? ('$' + Number(open.usage_cost.total_cost || 0).toFixed(2) + ' (~€' + Number(open.usage_cost.total_cost_eur || 0).toFixed(2) + ')') : '-')} <span class="sep">|</span> ${renderKv('today:', open.usage_cost?.available ? ('$' + Number(open.usage_cost.today_cost || 0).toFixed(2) + ' (~€' + Number(open.usage_cost.today_cost_eur || 0).toFixed(2) + ')') : '-')}</div>
        </div>
        <div class="agentMeta2">${(open.usage?.providers||[]).map(p => `${esc(p.display_name || p.provider || '-')}${p.plan ? ' ' + esc(p.plan) : ''}${(p.windows||[]).slice(0,2).map(w => ' | ' + esc(w.label) + ' ' + esc(Math.max(0, 100 - Number(w.used_percent ?? 0))) + '% left').join('')}`).join(' || ') || 'Keine Provider-Usage'}</div>
